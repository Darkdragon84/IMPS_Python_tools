...
class TestIMPS:
    ...
    def test_to_from_full_matrix(self, dim_phys, dims, dtype, seed):
        mps = MPSMat.random_mps(dim_phys, dims, dtype, seed)
        assert mps == MPSMat.from_full_matrix(mps.to_full_matrix(0), dim_phys, 0)
        assert mps == MPSMat.from_full_matrix(mps.to_full_matrix(1), dim_phys, 1)
    ...
    def test_mult_div_with_scalar(self, dim_phys, dims, dtype, seed, scalar):
        ...
        mps = MPSMat(mps_array)
        assert (scalar * mps) == MPSMat(mps_array * scalar)
        assert (mps * scalar) == MPSMat(mps_array * scalar)
        assert (mps / scalar) == MPSMat(mps_array / scalar)
    ...
    def test_mat_mult(self, dim_phys, dims, dtype_mat, dtype_mps, seed):
        ...
        mps = MPSMat.random_mps(dim_phys, dims, dtype_mps, seed)
        ...
        assert mpsl == MPSMat([matl @ mat for mat in mps.matrices])
        assert mpsr == MPSMat([mat @ matr for mat in mps.matrices])
